 ...
 * 6 other error
 */
static int hide_data_lsb_png(const char *input_filepath, struct binarydataarray hide_data,
 ...
 */
static int hide_data_lsb_bmp(const char *input_filepath, struct binarydataarray payload_data,
                             char *output_filepath);
 ...
 * @return  0 if the hiding process is successful, non-zero otherwise.
 */
static int extract_data_lsb_png(const char *input_image_filepath, struct binarydataarray *ptr_return_hidden_data);
 ...
 * @return  0 if the hiding process is successful, non-zero otherwise.
 */
static int extract_data_lsb_bmp(const char *input_image_filepath, struct binarydataarray *ptr_return_hidden_data);
...
static int hide_data_lsb_png(const char *input_filepath, struct binarydataarray hide_data,
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < row_bytes; x += 3) {
            ...
            is_success = binarydataarray_get_next_bit(hide_data, &payload_array_index,
...
static int hide_data_lsb_bmp(const char *input_filepath, struct binarydataarray payload_data,
    ...
    for (int y = 0; y < bmp_metadata.height; y++) {
        ...
        for (int x = 0; x < bmp_metadata.width; x++) {
            ...
            //change last bit of b_value to ptr_hidden_data
            bool next_payload_bit;
            is_success = binarydataarray_get_next_bit(payload_data, &payload_array_index,
                                                      &payload_element_bitshift, &next_payload_bit);
            if (!is_success) {
                LOG_MESSAGE(ERROR, "Cannot get next bit");
                fclose(input_file);
