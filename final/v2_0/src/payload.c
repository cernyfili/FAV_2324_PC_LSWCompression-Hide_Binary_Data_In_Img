 ...
 */
static bool
get_array_from_payload_file(const char *filename, struct payloadarray *ptr_return_output);
...
bool prepare_payload_data(const char *filename, struct payloadarray *ptr_return_output) {
    ...
    LOG_MESSAGE(WARNING, "Payload data size: %zu bytes", payload_data.length * sizeof(payload_type));
    ...
    LOG_MESSAGE(WARNING, "Compressed payload data size: %zu bytes", compressed_payload.length * sizeof(payload_type));
    ...
    if (final_size_bytes > UINT32_MAX){
    ...
    ptr_return_output->length += payloadsize_size / sizeof(payload_type);
 ...
 * 6 other error
 */
int extract_payload_from_data(struct payloadarray hidden_data, char **ptr_return_payload) {
    //SANITY CHECK
