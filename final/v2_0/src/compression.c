...
lzw_decompress(const struct staticdiccodearray compressed_data, struct dicvaluearray *ptr_return_decompressed_data) {
    ...
    for (size_t i = 1; i < compressed_data.length; i++) {
        ...
        if (!is_in_dictionary) {
            ...
            strcpy(s_value, (char *) old_value);
            size_t index = strlen((char *) old_value);
            if (index >= MAX_CODE_LENGTH) {
                LOG_MESSAGE(ERROR, "Code length is too long.");
                dictionary_free(&dictionary);
                dicvaluearray_free(&result_data);
                return false;
            }
            s_value[index] = STRING_NULL_TERMINATOR;
            //endregion

            //region LZW S = S + C
            if (c_value != STRING_NULL_TERMINATOR) {//is not last iteration
                s_value[index] = c_value;
                index++;
                if (index >= MAX_CODE_LENGTH) {
                    LOG_MESSAGE(ERROR, "Code length is too long.");
                    dictionary_free(&dictionary);
            ...
            //s_value = new_value
            strcpy(s_value, (char *) new_value);
            size_t index_new = strlen((char *) new_value);
            if (index_new >= MAX_CODE_LENGTH) {
                LOG_MESSAGE(ERROR, "Code length is too long.");
                dictionary_free(&dictionary);
                dicvaluearray_free(&result_data);
                return false;
            }
            s_value[index_new] = STRING_NULL_TERMINATOR;

            //endregion
        }

        //region LZW save to result S
        is_success = dicvaluearray_add_element(&result_data, (dic_value_type) s_value);
        if (!is_success) {
            LOG_MESSAGE(ERROR, "Memory allocation failed.");
            dictionary_free(&dictionary);
            dicvaluearray_free(&result_data);
        ...
        size_t index_old_value = strlen((char *) old_value);
        if (index_old_value >= MAX_CODE_LENGTH) {
            LOG_MESSAGE(ERROR, "Code length is too long.");
            dictionary_free(&dictionary);
            dicvaluearray_free(&result_data);
            return false;
        }
        old_value[index_old_value] = c_value;
        index_old_value++;
        if (index_old_value >= MAX_CODE_LENGTH) {
            LOG_MESSAGE(ERROR, "Code length is too long.");
            dictionary_free(&dictionary);
        ...
        old_value[index_old_value] = STRING_NULL_TERMINATOR;

        //save to dictionary
        struct dictionaryentry entry = {dictionary.length, (dic_value_type)old_value};
