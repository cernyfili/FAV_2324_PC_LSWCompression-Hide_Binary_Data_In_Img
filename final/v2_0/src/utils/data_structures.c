 ...
 * @return true if the addition is successful, false otherwise.
 */
static bool payloadarray_add_element(struct payloadarray *payload_array, payload_type element);
//endregion
...
/**
 * Initializes a struct payloadarray.
 *
 * @param data The struct payloadarray to be initialized.
...
bool payloadarray_initialize(struct payloadarray *data) {
    if (!data) {
        LOG_MESSAGE(ERROR, "struct payloadarray is NULL!");
        return false;
    }

    data->length = 0;
    data->array = TRACKED_MALLOC(sizeof(payload_type) * INITIAL_PAYLOADARRAY_SIZE);
    if (!data->array) {
...
/**
 * Adds a bit to a struct payloadarray.
 *
 * @param data The struct payloadarray to which the bit is added.
...
bool payload_array_add_bit(struct payloadarray *data, bool bit, size_t *array_index, size_t *element_bitshift) {
    ...
    if ((*element_bitshift) >= (size_t) (sizeof(payload_type) * BITS_IN_BYTE)) {
    ...
    if (data->capacity == 0) {
        is_success = payloadarray_initialize(data);
        if (!is_success) {
    ...
    if (*array_index >= data->length) {
        ...
        payload_type new_element = 0;
        is_success = payloadarray_add_element(data, new_element);
        if (!is_success) {
    ...
    size_t last_bit_shift = (size_t) ((sizeof(payload_type) * BITS_IN_BYTE) - 1);
...
/**
 * Adds an element to a struct payloadarray.
 *
 * @param payload_array The struct payloadarray to which the element is added.
...
static bool payloadarray_add_element(struct payloadarray *payload_array, payload_type element) {
    ...
    //if value doesnt fit
    if (payload_array->length == payload_array->capacity) {
        size_t new_capacity = payload_array->capacity + DIC_ARR_INC;

        void *temp_ptr = TRACKED_MALLOC(new_capacity * sizeof(payload_type));
        if (temp_ptr == NULL) {
            LOG_MESSAGE(ERROR, "Memory allocation failed.");
            TRACKED_FREE(payload_array->array);
            return false; // Memory allocation failed
        }
        memcpy(temp_ptr, payload_array->array, payload_array->capacity * sizeof(payload_type));
        TRACKED_FREE(payload_array->array);
        payload_array->array = temp_ptr;
        payload_array->capacity = new_capacity;
        temp_ptr = NULL;
    }

    payload_array->array[payload_array->length] = element;
    (payload_array->length)++;

    return true;

    /*
    return dynamicarray_add_element((struct dynamicarray *) payload_array, &element, sizeof(payload_type));*/
}

/**
 * Retrieves the next bit from a struct payloadarray.
 *
 * @param payload_array The struct payloadarray from which the bit is retrieved.
 * @param array_index Pointer to the current index in the array.
...
bool payloadarray_get_next_bit(struct payloadarray payload_array, size_t *array_index, size_t *element_bit_shift,
    ...
    if (*element_bit_shift >= (size_t) (sizeof(payload_type) * BITS_IN_BYTE)) {
