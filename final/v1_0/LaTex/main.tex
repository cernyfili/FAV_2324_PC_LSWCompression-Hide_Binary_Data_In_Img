...
% UVOD
    ...
    \chapter{Úvod}
    Tento dokument popisuje a definuje řešení semestrální práce z předmětu \textit{KIV/PC} na zadání: \textbf{Digitální steganografie}. Je zde uvedeno podle jakého zadání byla práce vypracována. Také je tu uvedená analýza úlohu, kde je podrobný rozbor problému, které jsou potřeba vyřešit pro naprogramování této práce. A následně popis řešení těchto problémů. Popsány jsou zde také využité algoritmy pro vyřešení problémů a jejich následná implementace a popis implementace celého řešení. Je zde i uživatelská příručka, která popisuje jak program přeložit a sestavit na různých platformách. Popisuje také ovládání tohoto programu a ukazuje praktické příklady využití. Jako poslední je zde shrnutí dosažených výsledků a splnění zadání. Také možné vylepšení a shrnutí problémů, které se vyskytli.
...
% ZADANI
    ...
    Program se bude spouštět př́kazem\\
...
% ANALYZA ULOHY
    ...
    Při analýze úlohy vyvstává několik problémů, pro které je potřeba nalézt postup řešení.
    ...
    Jelikož jazyk C nenabízí datový typ pro reprezentaci binárních dat je potřeba vytvořit alternativu. Binární data by se dala reprezentovat \textit{int} datovou strukturou (0, 1) ale to by zvýšilo nepřehlednost kódu a výsledné pole těchto znaků by bylo velice velké a následná konverze dat jiných struktur by byla zbytečně náročná. Proto pro tento projekt byla zvoleno dynamické pole \textit{char} se statickou inkrementací při nedostatku místa jelikož char je nejmenší možná datová struktura která může Výhody tohoto řešení jsou že se soubor jednoduše načítá do tohoto pole a v okamžiku konverze na jiný typ dat lze jednoduše provést konverzi jen pomocí přetypování reprezentace pointru.

    Pro pole mohlo být zvolen linked list nebo statické pole s dostatečnou velikostí nebo efektivnější dynamické pole (zvyšování o 2x násobek velikosti). Ale byl zvolno dynamické pole se zvyšováním velikosti o pevnou hodnotu jelikož implementace této struktury je jednoduché a díky tomu není náchilná na vytvoření chyb. Nejvhodnější by byl zvolit nejspíše linked list jelikož nezabírá v paměti více místa než je potřeba a není nikdy potřeba dostat se rychle k indexu pole a pouze se prochází celé pole od začátku dokonce.

    \subsection{Reprezentace slovníku LZW}
    Pro názornost a jednoduchost v kódu je slovník LZW reprezentován jako pole s char (hodnoty LZW slovníku) a int16 (hodnoty kódu slovníku) a toto pole je implementováno jako dynamické pole se pevnou inkrementací o konstantu. Alternativy tohoto řešení byla hash mapa, linked list nebo dynamické pole s jiným způsobem inkrementace. Pro toto využití by byla ideální využít hash mapa pro rychlí přístup buď k hodnotě kódu nebo kódu hodnoty, ale kvůli již předešle vytvořené implementaci dynamického pole a náročnosti implementace hash mapy bylo zvoleno pole. Jeho nevýhode je pomalejší vyhledávání kódu k hodnotě nebo hodnoty ke kódu ale pro toto využití dostačující. Linked list nebyl zvolen právě proto že je potřeba rychle přistupovat k hodnotám s indexem (kódem) a v tomto by byl linked list pomalejší.


    \section{Kontrola porušenosti dat payloadu}
    V rámci řešení bylo potřeba zvolit způsob jak zajistit že načtená data z obrázku nebyla poškozena manipulací s obrázkem. Je několik možností checksum
...
\subsection{Závěr}
    Jelikož Parity byte a sum complement nejsou dostačující pro ověření jestli nebyla data výrazně poškozena zvolil jsem crc přímo crc32 díky předešlé znalosti této checksumy a její implementace a taktéž jelikož díky knihovně \textit{zlib.h} je implementace do programu rychlá. SHA a MD5 nebyli zvoleny kvůli náročnosti implementace do tohoto projektu i když jsou to schopné alternativy.
...
END WHILE
    ...
    Tento modul zajišťuje zaprvé zaobalení dat které mají být schovány se signaturou a kontrolní součet a následně využije modul \textit{Compression} pro kompresy schovaných dat. Tento modul vytváří kontrolní součet na základě zkomprimovaných dat pomocí CRC32 viz.

    A zadruhé získání pouze schovaných dat z dat která byla extrahována z obrázku a to odstraněním signatury a kontrolního součtu a následné dekomprese pomocí modulu \textit{Compression}. Tento modul současně kontroluje jestli je signatura správná a jestli sedí kontrolní součet neboli jestli data nebyla poškozená

    Struktura dat ukládaných do obrázku
    -- Tabulka jak je poskládaný payload file

    signature - 38 bytu
    velikost schovaných dat v bytech - 4 byty
    kontrolní součet - 4 byty
    payload data - podle velikosti schovaných dat


    \section{Pomocný modul: Data Structures - datová struktura pro binární data}
    Tento modul implementuje strukturu \textit{payloadarray} která reprezentuje binární data předávaná mezi moduly. Je realizována jako dynamické pole char kde každý element tohoto pole vyjadřuje 8 binárních znaků (jelikož char má velikost 1 byte). Je zde i funkce která vrací jednotlivé bity, které využívají bitové operátory pro získání specifického bytu z elementu. Také je zde funkce pro přidávání jednotlivých bytů do pole která postupně upravuje jednotlivé bity v elementech a přidává elementy podle potřeby pomocí bitových operátorů. Pole je realizováno jako dynamické a v okamžiku že je plné je inkrementováno o konstantu 1000 a realokována paměť.


    \section{Pomocný modul: Dictionary - datové strutury pro kompresy}
    Tento modul implementuje datové struktury pro slovník LZW algoritmu.
    Je to realizováno jako struktura dynamického pole která za elementy obsahuje strkturu která představuje jeden slovníkový zápis a tento slovníkový element je tvořen z kódu (relizováno jako int 16 bitů) a hodnoty (realizováno jako řetězec). V okamžiku je je pole slovníkovových zápisů plné zvýší se kapacita pole o konstantu a je realokována paměť.

    Pro reprezentaci polí slovníkových hodnot se taktéž používá dynamické pole s zvýšením kapacity o konstantu.
    Pro reprzentaci pole kódových hodnot se používá dynamické pole se statickou velikostí.
    Tyto struktury se využívájí pro komunikaci mezi funkcemi v modulu \textit{Compression}
...
Files doesn't exist or cannot be open.
exit code 1
        \end{minted}
    \end{mdframed}
...
exit code 3
    ...
    \begin{mdframed}
        \begin{minted}[linenos]{shell}
Unable to extract the payload from the image.
Image doesn't have payload inside.
exit code 4
        \end{minted}
    \end{mdframed}
...
% ZAVER
    ...
    \begin{itemize}
        ...
        \item Payload soubor: 10 B
    \end{itemize}

    \paragraph{Výsledky}
    \begin{itemize}
        \item Ukrytí do BMP souboru: 14.123 ms
        \item Ukrytí do PNG souboru: 10.739 ms
        \item Extrakce z BMP souboru: 7.516 ms
        \item Extrakce z PNG souboru: 7.048 ms
    \end{itemize}
