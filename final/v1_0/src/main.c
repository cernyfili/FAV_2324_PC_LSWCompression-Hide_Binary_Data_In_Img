...
hide_payload(const char *input_image_filepath, const char *hide_payload_filepath) {
    ...
    char *temp_output_image_filepath = "temp";/*
    char* * pointer = &temp_output_image_filepath;
...
static int extract_payload(const char *input_image_filepath, const char *output_payload_filepath) {
    ...
    // Extract the payload from the image
    struct payloadarray hidden_data;
    //Prepare ptr_hidden_data
    /*bool is_success = payloadarray_initialize(&hidden_data);
    if (!is_success) {
        LOG_MESSAGE(ERROR, "Unable to initialize the payload array.");
        return 6;
    }*/

    hidden_data.length = 0;
    hidden_data.capacity = 1000;
    hidden_data.array = TRACKED_MALLOC(sizeof(payload_type) * hidden_data.capacity);
    ...
    char *payload_data;
    result = extract_payload_from_data(hidden_data.array, &payload_data);
    if (result != 0) {
